...
| chibireact                | Reactで対応する主な領域                     |
| ------------------------- | ------------------------------------------- |
| `packages/chibireact`     | `packages/react`                            |
| `packages/reconciler`     | `packages/react-reconciler`                 |
| `packages/chibireact-dom` | `packages/react-dom-bindings` / `react-dom` |
| `packages/scheduler`      | `packages/scheduler`                        |

完全な互換実装ではありません。ここから本家ソースを読む際の地図として利用してください。

完成版: [`packages`](https://github.com/beryu/chibireact/tree/main/packages) · [Playground](https://github.com/beryu/chibireact/tree/main/examples/playground)
