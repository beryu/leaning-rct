...
type RctElement = {
  ...
  props: { children?: RctNode };
};
```

`$$typeof`はleaning-rctのElementであることを識別し、`type`は作る対象、`props`は入力、`key`は兄弟間の同一性を表します。
...
root.render(createElement("h1", null, "Hello, leaning-rct!"));
```
...
React 19ではクライアントrootを`react-dom/client`の`createRoot`で作ります。leaning-rctもcoreとDOM入口を分離し、後で別rendererを作れる境界を残します。

章の実装: [`book/impls/10-first-render`](https://github.com/beryu/leaning-rct/tree/main/book/impls/10-first-render)
